...
export class AttractionEngine {
  ...
  getWeakestSubcategoryGuidance(subId, clusterId) {
    const g = {
      ...
      performanceStatus: { meaning: 'This bar blends wealth/finance, visible status, productivity, talent, popularity, and visible mate-market traction (interest others could notice, follow-through when you engage, involuntary dry gaps)—not the dedicated mate-market proof slice.', actions: ['Strengthen the financial picture: runway, debt control, and visible trajectory—not only headline income', 'Raise verifiable credibility: credentials, portfolio, wins, or audience that outsiders can check', 'Increase social leadership reps: host plans, set logistics, follow through, and showcase one standout talent'] },
      mateMarketProof: { meaning: 'Mate-market proof is a small axis slice for self-reported quality of who showed interest versus your realistic ceiling, how observable that interest was, and whether it came from independent sources—distinct from generic popularity or wealth signals.', actions: ['Tighten the observability loop: one public-facing channel (social, hobby scene, or reputation) where interest is legible without oversharing', 'Broaden pipelines: separate scenes so interest is not only one overlapping circle', 'Calibrate honesty on partner calibre versus your physical and status fundamentals—large gaps read as uncertainty, not leverage'] },
      ...
      riskCost: { meaning: 'Risk Cost indicators (volatility, red flags, drama, and skew toward casual versus committed relationship history) filter men out of long-term investment.', actions: ['Reduce visible red flags: substance use, instability, destructive patterns', 'Increase predictability — stable routine, consistent mood, clear communication', 'If your history skews casual, signal intent and boundaries earlier with men you want to keep—ambiguity reads as cost'] },
  ...
  /** Male-only social-proof read (light-weight signal from perf_9..perf_11 + mmp_*). */
  getMaleSocialProofSignalLine(rawResponses) {
    if (this.currentGender !== 'male') return '';
    const responseMap = rawResponses || {};
    const socialProofItems = [
      { id: 'perf_9', weight: 0.65 },
      { id: 'perf_10', weight: 0.65 },
      { id: 'perf_11', weight: 0.65 },
      { id: 'mmp_1', weight: 0.95 },
      { id: 'mmp_2', weight: 0.95 },
      { id: 'mmp_3', weight: 0.95 }
    ];
    ...
    return `Mate-market / social proof signal: ${intensity} (~${percentile}th percentile) from visible interest, follow-through when you engage, dry-gap pattern, plus calibre / observability / source variety—summarised for the report; the axis blends these into performanceStatus and Mate-market proof with conservative weights.`;
  }
